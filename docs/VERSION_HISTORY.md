...
- 改动：员工首页新增“今日报工”面板，复用 `order.getAssignedProcesses` 展示本人负责工序卡片，点选后可在首页填写数量并调用 `worklog.submit` 提交；工序卡展示订单、工价/隐藏状态、剩余可报、已报数量和进度；完整报工页支持 `process_id` 入参并自动选中对应工序。
- 数据影响：无新增集合或字段；首页提交仍写入既有 `WorkLogs`，继续沿用后端工序归属、订单状态、剩余额度和订阅限制校验。
- 验证：新增员工首页工序展示与数量状态纯逻辑测试；需在微信开发者工具确认首页工序选择、数量输入、提交成功后余额和今日预估工资刷新。
