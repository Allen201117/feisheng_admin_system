...
- 顺带去掉薪资列表的信息重复（2026-08-29 老板确认）：每行左边的绿色勾选框和右边的「已发/未发」徽章是同一个信息，徽章还占掉这一行本就不宽的空间，把请假徽章挤换行。现在**发放状态只由勾选框表达**，列表标题旁加了「打勾 = 已发薪」的说明。总数仍看顶部「已发 N/M 人」；按发放状态搜索不受影响（`SALARY_SEARCH_FIELDS` 里那一项是取数据字段算的，不依赖徽章文字）。注意「员工工资档案」页（employee-salary）没有勾选框，那里的「已发/未发」徽章保留。
