...
- `pages/boss/settings/settings`（2026-08-29 整理）：由 7 张同级卡片平铺改为**三段分组**——「考勤打卡」(打卡点/打卡规则)、「工资与统计」(发薪机制/合格率阈值/数据体检)、「账号与服务」(续费/隐私政策/服务协议)。卡片统一 `settings-head` + `settings-desc` + `settings-row` 结构，内联样式全部收进 `settings.wxss`。**数据体检**（结算价对账修复入口，见 VERSION_HISTORY 2026-08-29）三种明确状态：绿=全部对齐 / 橙=查出差异待确认 / 绿=刚写完库，配 toast 反馈。
