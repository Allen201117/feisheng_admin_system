...
- `pages/boss/settings/settings`（2026-08-29 整理）：由 7 张同级卡片平铺改为**三段分组**——「考勤打卡」(打卡点/打卡规则)、「工资与统计」(发薪机制/合格率阈值/数据体检)、「账号与服务」(续费/隐私政策/服务协议)。卡片统一 `settings-head` + `settings-desc` + `settings-row` 结构，内联样式全部收进 `settings.wxss`。新增「数据维护」段，含两个一次性修复入口：**结算价对账**（`salary.repairSettlementPrices`）与**历史半天请假**（`attendance.repairLegacyHalfDayLeaves`），都是三种明确状态：绿=没问题 / 橙=查出差异待确认 / 绿=刚写完库，配 toast 反馈，默认 dry_run 只看不改。
- `pages/boss/attendance/attendance`（2026-08-29 重做）：**按月的员工考勤列表**。顶部 ‹ 月份 › 切换 + 本月概览（全勤人数/不全勤/今日出勤/今日请假）；列表每行=员工+「出勤N天·请假N天·缺勤N天」+全勤徽章（绿=全勤 / 红=不全勤，口径见 §2.10），不全勤的排最前；点员工弹**日历抽屉**：当月每天绿=出勤 / 红=请假（角标「假」）/ 红绿对半=请半天（角标「上」「下」）/ 不上色=还没到，下附当月请假明细（日期+原因+来源）。另有「异常补签」tab 保留原有批量补签入口。**考勤口径不看打卡、只看请假**（见 §2.10）。日历渲染纯函数在 `utils/attendance-calendar.logic.js`。
...
- `attendance`：`clockIn`、`clockOut`、`getTodayRecord`、`getMonthlyHours`、`getDailyRecords`、`getPeriodRecords`、`getAbnormalRecords`、`supplement`、`getUserMonthlyRecords`、`checkAbnormal`、`updateLeaveHalfDay`（老板直接把一条请假改成 全天/上午/下午/半天 并重算 day_count，作用于该记录所有日期）、`repairLegacyHalfDayLeaves`（老数据迁移：从请假备注里认出「上午/下午/半天」补 `half_days` + 重算 `day_count`，boss only、默认 dry_run、幂等，纯逻辑 `planLegacyHalfDayMigration`）、`getMonthAttendanceOverview`（老板端月度总览：一次返回本月全部员工的出勤/请假统计 + 每人每天状态清单 + 请假明细，只查 Users+LeaveRecords 不查 Attendances，纯逻辑 `attendance/attendance-summary.logic.js`）；**请假（LeaveRecords）**：`requestLeave`、`cancelLeave`、`getMyLeaves`（员工）、`getMonthLeaveSummary`、`getLeaveRequestsForBoss`、`getUnreadLeaveCount`、`getTodayLeaveCount`（今日请假去重人数，供首页考勤卡）、`markLeavesRead`、`bossAddLeave`（老板代员工补录，校验目标员工同 org、允许过去日期、标记 `created_by_boss`+操作人、自动已读）、`bossDeleteLeave`（老板删任意本厂请假=员工提报+代录，软删 status→cancelled，不限本人/日期）。纯逻辑见 `attendance/leave.logic.js`（日期口径）与前端 `utils/leave-calendar.logic.js`（日历渲染，`allowPast` 控制能否选过去日期）。**全勤口径不分来源**：只按 `status==='active'` 统计，老板代录与员工提报同等计入，删除(cancelled)不计入。**全勤 = 当月请假合计 ≤ 2 天**（2026-08-29 口径，半天记 0.5 天），阈值真源 `attendance/leave.logic.js` 的 `FULL_ATTENDANCE_MAX_LEAVE_DAYS`，前端同口径副本在 `utils/attendance-calendar.logic.js`（`tests/attendance-calendar.logic.test.js` 校验两边一致）。
