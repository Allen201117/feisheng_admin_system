...
- `pages/boss/attendance/attendance`（2026-08-29 重做）：**按月的员工考勤列表**。顶部 ‹ 月份 › 切换 + 本月概览（全勤人数/不全勤/今日出勤/今日请假）；列表每行=员工+「出勤N天·请假N天·缺勤N天」+全勤徽章（绿=全勤 / 红=不全勤，口径见 §2.10），不全勤的排最前；点员工弹**日历抽屉**：当月每天绿=出勤 / 橙=请假（角标 假/上/下）/ 红=缺勤 / 灰=未到，下附当月明细（签到签退时间）与异常日补签。另有「异常补签」tab 保留原有批量补签入口。日历渲染纯函数在 `utils/attendance-calendar.logic.js`。
