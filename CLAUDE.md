...
- **请假支持半天**：`LeaveRecords.half_days = { 'YYYY-MM-DD': 'am' | 'pm' | 'half' }`，半天记 **0.5 天**；没有该字段的老记录一律当全天，向后兼容。前端交互是**连点同一天循环**：未选 → 全天 → 上午 → 下午 → 未选。
  - `'half'` = **只知道是半天、不知道哪半天**，由老数据迁移（`detectHalfDayFromReason` 只读出「半天」二字）或老板手动指定产生。日历标「半」角标。**宁可标不确定也不瞎猜上午还是下午。**
  - **交互是三个显式按钮「全天 / 上午 / 下午」**（`LEAVE_KINDS` + `applyLeaveKind`），先选时段再点日期；点已选中且同时段的日期=取消，点已选中但不同时段的=直接改过去。一次请假可以混着选（3 号全天 + 5 号上午）。此前的「连点同一天循环切换」已废弃——点错了要绕一圈才回得来。
- **老板可直接改一条请假的时段**：`attendance.updateLeaveHalfDay`（boss only，作用于该记录的**所有**日期），入口在「请假提醒列表」每行的「改时段」和「设置 → 数据维护 → 历史半天请假」的读不出清单里。一条记录里有的日子全天、有的半天，仍需删了分开重录。
- ⚠️ **请假天数必须按「去重后的日期」算，不能把多条记录的 `day_count` 直接相加**（`summarizeMonthLeave`）。线上真实存在同一员工同一天被两条请假记录覆盖的情况（先录 6/23+6/24，又单独录一条 6/23），相加会把 2 天算成 3 天，把本来全勤的人误判成不全勤。只有「没有 dates 只有 day_count」的脏记录才走累加兜底。
