...
> 给在本仓库工作的 AI 代理（Codex / Codex 等）。**动手改任何代码前，先读完本文件**，再按需读
...
2. **不复制第 N 套统计**：工资/排行榜/数据中心/导出的聚合口径**已经有 4 套实现**（salary、period-statistics、export、leaderboard 各一份 `quantity*有效snapshot_price`）。只能复用或抽公共纯函数，**禁止再加一套**。
...
### 2.1 计件工资口径 ✅
- 工资 = `Σ(WorkLogs.quantity × 有效snapshot_price)`，**按报工数量，不按合格数 `passed_qty`**。`passed_qty` 只用于质量统计（合格率），**永不进工资**。已确认，别再问。
- `snapshot_price` 是 WorkLog 上的结算单价字段；**未发薪报工的有效结算价必须跟随 `Processes.current_price`**（读/算薪时可同步纠正历史漏同步旧价），**已发薪后才冻结历史快照价**。
...
- ✅ 已实现（2026-06-11/2026-06-29）：`order.updateProcessPrice` / `updateProcess` 改价前先 `findProcessPaidWorklogConflicts`（`pay-lock.logic.js`，按月+按订单两种发薪都查），命中任一已发薪报工**整体拒绝**并提示「该工序已发薪，单价不可修改」；未发薪走 `syncZeroPriceWorklogsForProcess` 同步重写（注意：按**未发薪**过滤，不是按零价，函数名有误导），成功文案已改为「未发薪报工已同步为新单价」。为兼容历史漏同步数据，`settlement-price.logic.js` 已接入 worklog/salary/export/leaderboard：未发薪读算按当前价，已发薪保留快照价。
...
- 工资金额仍按有效 `snapshot_price` 结算（§2.1 不变）：未发薪有效价跟随当前价，已发薪保留历史快照价。实时工价是展示字段；当已发薪历史 `snapshot_price ≠ current_price` 时，明细要同时标清「结算价(snapshot) / 当前价(current)」，避免老板困惑。
- 适用面：data-center、export 各报表（报工核算表 / 工资核算表）、salary / salary-detail、worklog-manage、order-detail 的报工/工资展示。
- ✅ 已实现（2026-06-11/2026-06-29）：`export`（月/年明细表新增「结算单价/当前工价」两列；订单矩阵表工价括号本就是 current_price）、`worklog.getManageLogs` 与 `salary.getUserMonthlySalaryByBoss` 返回 `current_price`+`price_changed`（按 process_id 批量 join，无 N+1），worklog-manage / salary-detail 明细行显示结算价/当前价。2026-06-29 起，未发薪记录返回的结算价会先同步为当前价；只有已发薪历史才继续展示结算价与当前价差异。

---

## 3. 业务闭环 & 角色

**闭环**：老板建订单+工序（定工价）→ 分配员工 → 员工报工（写入 `snapshot_price`）→ QC 填 `passed_qty` 质检 → 工资按 `quantity×有效snapshot_price` 核算（未发薪跟随当前价，已发薪冻结快照价）→ 发薪锁定 → 统计/排行/数据中心 → Excel 导出。并行：考勤打卡（GPS 地理围栏 + 二维码）→ 工时；多工厂隔离；订阅到期温和拦截。
...
迭代记录看 `docs/VERSION_HISTORY.md`；我的跨会话记忆（含老板交接的口径与在做的任务）在 `~/.Codex/.../memory/MEMORY.md`。
...
**规则优先级**：当前会话明确指令 > 本文件 §2 业务口径 > 项目其他文档 > 全局 AGENTS.md > 默认行为。
